...
describe("Campaigns", () => {
  ...
  it("allows making a request", async () => {
    await campaign.methods
      .createRequest("d", "100", accounts[1])
      .send({ from: accounts[0], gas: "1000000" });
    const request = await campaign.methods.requests(0).call();

    assert.strictEqual(request.description, "d");
  });
  ...
  it("process requests", async () => {
    ...
    await campaign.methods
      .createRequest("d", web3.utils.toWei("5", "ether"), accounts[1])
      .send({ from: accounts[0], gas: "1000000" });
    await campaign.methods
      .approveRequests(0)
      .send({ from: accounts[0], gas: "1000000" });
